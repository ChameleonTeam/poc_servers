...
import (
	...
	"dummyServer/userCommons"
)

func Connect() (*sql.DB, error) {
	db, err := sql.Open("sqlite3", "./USERS.db")
	if err != nil { return nil, err}
	if db == nil { return nil, fmt.Errorf("DB nil") }
	return db, err
}
...
func insertUser(db *sql.DB, pr *userCommons.PersonRequest) (*userCommons.PersonRequest, error) {
...
func getUser(db *sql.DB, dni string) (*userCommons.PersonRequest, error){
	
	var person userCommons.PersonRequest
...
func listUsers(db *sql.DB) ([]userCommons.PersonRequest, error){
	
	personList := make([]userCommons.PersonRequest, 0)
